...
object CombiGrid {
  ...
  //Main. Used to test and verify output.
...
def hierRecThreadsWhileLoops(si:Int, ti:Int, centeri:Int, interval:Content, leveli:Int){
  ...
  if(localSize == 0) {
    if(ic.l(0) <= 0) {
      ...
      var i=s
      while(i < t) { // Counts from s to t-1
        var rmask = myPow2(i)
        var dist = myPow2(-ic.l(i))
        var lVal:Double=0; 
        var rVal:Double=0; 
        if((ic.l(6) & rmask)!=0) { //Checks if the bitwise combination is not all 0.
          var posLeft = center - dist*strides(i);
          var lVal = grid(posLeft);
        }
        else {
